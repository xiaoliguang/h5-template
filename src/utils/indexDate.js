...
const MIN_DATE = new Date(NOW_DATE.getFullYear() - 60 , 0, 1); // 最小支持日期 60年前
const MAX_DATE = new Date(NOW_DATE.getFullYear() - 18, 11, 31); // 最大支持日期 18年前

const transformDateToIndex = (ARG_DATE) => {
  let isnull = !ARG_DATE;
  ARG_DATE = ARG_DATE||NOW_DATE;
  let defaultDateIndex = [];
  defaultDateIndex.push(ARG_DATE.getFullYear() - MIN_DATE.getFullYear()-(isnull?18:0)); // 年
...
export { transformDateToIndex, getDatePickerIndex, getDays, fixPickerValueDefault,MIN_DATE,NOW_DATE,MAX_DATE };
